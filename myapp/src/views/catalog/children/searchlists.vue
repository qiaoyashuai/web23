...
export default {
  ...
  methods: {
    ...
    reqdatas () {
      ...
      console.log(parmas);
      this.$store.dispatch("changecataloglistsdata", parmas)//调用store里面的actions方法 为state中的数据赋值
      setTimeout(() => {
        console.log(this.$store.state.cataloglistsdata);
      }, 1000)
    }
  ...
  mounted () {
    ...
    this.reqdatas()//获取数据 执行方法获取数据 为状态管理中的 cataloglistsdata 赋值然后通过计算属性来给页面中的赋值
    this.fonscolor()
